...
SELECT setor.nomeSetor, funcionario.* FROM funcionario JOIN setor ON fksetor = idSetor WHERE idFuncionario IN(1,4);
SELECT * FROM funcionario JOIN acompanhante ON fkfuncionario = idFuncionario;
SELECT funcionario.nome, acompanhante.* FROM acompanhante JOIN funcionario ON fkfuncionario = idfuncionario WHERE idfuncionario = 1;
SELECT * FROM funcionario JOIN setor ON idsetor= fksetor JOIN acompanhante ON  fkfuncionario = idfuncionario;
